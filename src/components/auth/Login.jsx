...
const Login = ({ onLoginSuccess, switchToSignup }) => {
  ...
  const handleSocialLogin = async (provider, providerName) => {
    ...
    try {
      ...
      const response = await axios.post(`${API}/api/auth/social`, {
        firebaseToken,
      ...
      if (err.code === 'auth/popup-closed-by-user') {
        ...
        setError(err.response?.data?.error || `${providerName} login failed. Please try again.`);
      }
    ...
    <div className="auth-container">
      <div className="auth-card">
        ...
        <div className="social-login-section">
          ...
          >
            ...
            <div className="button-text">
              ...
              <span className="sub-text">Continue with Google or X</span>
            </div>
          </button>
        </div>
        ...
        <div className="auth-footer">
          <p>
            Wala pang account?{' '}
            <button onClick={switchToSignup} className="auth-link">Sign up here</button>
          </p>
        </div>
      </div>
        ...
        <div className="modal-overlay" onClick={() => setShowSocialModal(false)}>
          <div className="social-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="social-buttons">
              ...
              >
                {socialLoading === 'Google' ? <span>Connecting...</span> : (
                  <>
              ...
              <button
                ...
                disabled={!!socialLoading}
              >
                {socialLoading === 'X' ? <span>Connecting...</span> : (
                  <>
                    <svg viewBox="0 0 24 24" className="social-icon">
              ...
              <button className="social-button facebook disabled" disabled>
                <svg viewBox="0 0 24 24" className="social-icon">
                  <path fill="#1877F2" d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                </svg>
                Continue with Facebook <span className="coming-soon-tag">Soon</span>
              </button>

              <button className="social-button apple disabled" disabled>
                <svg viewBox="0 0 24 24" className="social-icon">
                  <path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.8 1.18-.24 2.31-.93 3.57-.84 1.51.12 2.65.72 3.4 1.8-3.12 1.87-2.38 5.98.48 7.13-.57 1.5-1.31 2.99-2.54 4.09l.01-.01zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.29 2.58-2.34 4.5-3.74 4.25z"/>
                </svg>
                Continue with Apple <span className="coming-soon-tag">Soon</span>
              </button>
            </div>
          </div>
        </div>
